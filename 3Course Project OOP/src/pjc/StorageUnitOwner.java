...
public class StorageUnitOwner {
	...
	public void HireAgents() {
		...
		Transaction t = session.beginTransaction();
		Query query = session.createQuery("FROM  StorageUnit");
	    List listUnits = query.list();
	    
	    query = session.createQuery("FROM  StorageUnitAgent");
	    List listAgents = query.list();
	    
		
		System.out.println("Number of Agents to Hire: ");
		nAgents = sc.nextInt();
		
		System.out.println("List of Agents");
	    for (int i = 0; i < listAgents.size(); i++) {
	    	System.out.println((i + 1) + ". " + ((StorageUnitAgent) listAgents.get(i)).toString());
		...
		while (nAgents > 0) {
			...
			ID_Agent = sc.nextInt();
			((StorageUnit) listUnits.get(--ID_Unit)).SetContractor((StorageUnitAgent) listAgents.get(--ID_Agent));
			nAgents--;
		}
		t.commit();
		session.close();
